...
class Interpretator {
  public:
    ...
    inline void CreateFrame(int64_t func_id, int64_t n_args) {
        ...
        new_frame->SetPtr(function->_bytecode, function->_bytecode_len, function->_bytecode);
    ...
    inline void ReturnPreviousFrame() {
        int64_t returned_value = _current_frame->_return_value;
        delete _current_frame;
        _frames.pop();
        _current_frame = _frames.top();
        _current_frame->_return_value = returned_value;
